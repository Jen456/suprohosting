...
<div class="product-meta product_meta">
	...
	<?php if ($isLegacy || (!$isLegacy && $isCategories)): ?>
        <?php if (!$isLegacy): $title = $categoriesTitle ? '<span class="date-color">'.esc_html($categoriesTitle).': </span>' : null?>
		    <?php echo wc_get_product_category_list( $product->get_id(), ', ', '<div class="posted_in">'.$title.'<span>', '</span></div>' ); ?>
		<?php else:; ?>
            <?php echo wc_get_product_category_list( $product->get_id(), ', ', '<div class="posted_in"><span class="date-color">' . _n( 'Category:', 'Categories:', count( $product->get_category_ids() ), 'woocommerce' ) . '</span> <span>', '.</span></div>' ); ?>
	    <?php endif; ?>
    <?php endif; ?>
