...
<div class="form-field" style="display: none;">
	<label for="thegem_attribute_swatches_size"><?php esc_html_e( 'Size (px)', 'thegem' ); ?></label>
	<input type="number" name="thegem_attribute_swatches_size" id="thegem_attribute_swatches_size" />
	<script type="text/javascript">
	jQuery( document ).on( 'change', '#attribute_type', function( event ) {
		if(jQuery(this).val() === 'image' || jQuery(this).val() === 'color') {
			jQuery('#thegem_attribute_swatches_size').closest('.form-field').show();
		} else {
			jQuery('#thegem_attribute_swatches_size').closest('.form-field').hide();
		}
	});
	jQuery('#attribute_type').trigger('change');
	</script>
...
function thegem_woocommerce_after_edit_attribute_fields() {
	...
	$size = get_option('thegem_attribute_'.$att_name.'_swatches_size');
?>
...
<tr class="form-field" style="display: none;">
	...
	<td>
		<input type="number" name="thegem_attribute_swatches_size" id="thegem_attribute_swatches_size" value="<?php echo esc_attr($size); ?>" />
		<script type="text/javascript">
		jQuery( document ).on( 'change', '#attribute_type', function( event ) {
			if(jQuery(this).val() === 'image' || jQuery(this).val() === 'color') {
				jQuery('#thegem_attribute_swatches_size').closest('.form-field').show();
...
function thegem_woocommerce_attribute_image_save($id, $data, $old_slug = false) {
	if(!empty($old_slug)) {
		...
		delete_option('thegem_attribute_'.$old_slug.'_swatches_size');
	}
	...
	if($data['attribute_type'] === 'image' || $data['attribute_type'] === 'color') {
		$size = isset( $_POST['thegem_attribute_swatches_size'] ) ? $_POST['thegem_attribute_swatches_size'] : '';
		update_option('thegem_attribute_'.$data['attribute_name'].'_swatches_size', $size);
	}
}
...
function thegem_woocommerce_attribute_image_delete($id, $name, $taxonomy) {
	...
	delete_option('thegem_attribute_'.$name.'_swatches_size');
}
	...
	if(!empty($attribute_data) && ($attribute_data->type == 'color' || $attribute_data->type == 'image' || $attribute_data->type == 'label')) {
		...
		if ( ! empty( $options ) ) {
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				$size = intval(get_option('thegem_attribute_'.$attribute_slug.'_swatches_size'));

				$html .= '<ul class="styled gem-attribute-options"' . ($size > 0 ? ' style="--thegem-swatches-size: ' . $size . 'px;"' : '') . '>';
