...
class TheGem_Countdown extends Widget_Base {
	...
	protected function content_settings() {
		...
		$this->add_control(
			'site_timezone',
			[
				'label' => __('Use the website time zone', 'thegem'),
				'type' => Controls_Manager::SWITCHER,
				'default' => 'no',
				'description' => __('If disabled, the UTC time zone will be used', 'thegem'),
			]
		);
		$this->add_control(
			'starting_date',
...
//					'{{WRAPPER}} .countdown-style-3 .countdown-wrapper .countdown-item:not(:last-of-type)'  => 'margin-right: calc( {{SIZE}}{{UNIT}}/2 );',
	...
	public function render() {
		...
		$settings['content_layout'] = thegem_check_array_value(array('boxes', 'elegant', 'cross', 'days_only', 'circles'), $settings['content_layout'], 'boxes');
		$settings['position'] = thegem_check_array_value(array('flex-start', 'center', 'flex-end'), $settings['position'], 'center');
		...
		$timestamp = strtotime($settings['due_date']);
		$site_timezome = isset($settings['site_timezone']) && $settings['site_timezone'] === 'yes';
		if(!empty($site_timezome)) {
			$dt = new \DateTime($settings['due_date'], wp_timezone());
