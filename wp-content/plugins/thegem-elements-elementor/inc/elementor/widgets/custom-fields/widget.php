...
class TheGem_Custom_Fields extends Widget_Base
{
	...
	public function get_edit_template_type()
	{
		...
		if (thegem_get_template_type(get_the_ID()) === 'single-post' || thegem_get_template_type(get_the_ID()) === 'loop-item' || isset($GLOBALS['thegem_loop_item_post'])) {
			$type = 'post';
		}
	...
	public function render()
	{
		...
		switch ($this->get_edit_template_type()) {
			...
			case 'product':
				...
				$post_id = !empty($product) ? $product->get_id() : 0;
				break;
			default:
				$post_id = get_the_ID();
		}
							...
							if ($item['field_type'] == 'inherit'){
								...
								switch ($field_type) {
									...
									case 'video' :
										...
										break;
									case 'wysiwyg' :
										$value_output = '<div class="meta">' . wp_kses_post(wptexturize(wpautop($meta_value))) . '</div>';
										break;
									case 'checkbox' :
