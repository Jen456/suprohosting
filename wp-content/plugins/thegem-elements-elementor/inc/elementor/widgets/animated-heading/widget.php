...
class TheGem_Animated_Heading extends Widget_Base {
	...
	public function render() {
		...
		foreach ($text_content as $k => $value) {
			...
			if (!empty($rotating_text)) {
				...
				if ($settings['animation_enable'] == 'yes') {
					...
					$title .= '<span' . (!empty($inner_class) ? ' class="' . esc_attr(trim($inner_class)) . '"' : '') . (!empty($text_style) ? ' style="' . esc_attr(trim($text_style)) . '"' : '') . '>' . nl2br(esc_html(strip_tags($value['heading_text']))) . '</span> ';
				}
			}
		}
		...
		/*if(!in_array(Utils::validate_html_tag($settings['header_size']), array('h1', 'h2', 'h3', 'h4', 'h5', 'h6'))) {
			$this->add_render_attribute('title-fancy', 'role', 'heading');
		}*/
