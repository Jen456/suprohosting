					...
					while ( $persons->have_posts() ) {
						...
						if(!empty($settings['skin_source']) && $settings['skin_source'] === 'builder' && !empty($settings['loop_builder'])) {
							...
							if ( !empty( $thegem_link) ) {
								$hover_class = ' w-link';
								$thegem_link_start  = '<a class="team-person-link" href="' . esc_url( $thegem_link ) . '" target="' . esc_attr( thegem_get_data( $thegem_item_data, 'link_target' ) ) . '" aria-label="' . esc_attr($thegem_item_data['name']) . '">';
								$thegem_link_end    = '</a>';
							}else{
								$hover_class = '';
							}
