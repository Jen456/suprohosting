...
class TheGem_Extended_BlogCarousel extends Widget_Base {
	...
	public function render() {
		...
		$allowed_hover_effect = ['default', 'zooming-blur', 'horizontal-sliding', 'vertical-sliding', 'gradient', 'circular', 'zoom-overlay', 'disabled'];
		$image_hover_effect = isset($settings['image_hover_effect'])
			? sanitize_key($settings['image_hover_effect'])
			: 'default';
		if (!in_array($image_hover_effect, $allowed_hover_effect, true)) {
			$image_hover_effect = 'default';
		}

		$hover_effect = 'new-' . $image_hover_effect;
		wp_enqueue_style('thegem-news-grid-version-new-hovers-' . $image_hover_effect);
		...
		$settings['image_shadow_box_shadow_type'] = empty($settings['image_shadow_box_shadow_type']) ? 'no' : $settings['image_shadow_box_shadow_type'];
		...
		if ($news_grid_loop->have_posts() || !empty($categories_filter) || !empty($attributes_filter) || !empty($search_current)) {
			...
			if ($settings['disable_preloader'] == '') {
				if ($settings['columns_desktop'] == '100%' || $settings['skeleton_loader'] !== 'yes') {
					...
					echo apply_filters('thegem_portfolio_preloader_html', '<div class="preloader save-space"><div class="' . esc_attr($spin_class) . '"></div></div>');
				} else { ?>
			...
			<div class="portfolio-preloader-wrapper<?php echo !empty($settings['sidebar_position']) ? ' panel-sidebar-position-' . esc_attr($settings['sidebar_position']) : ''; ?>">
												...
												if ($news_grid_loop->have_posts()) {
													...
													<div class="portfolio-item not-found">
														<div class="found-wrap">
															...
															<div class="msg">
																<?php echo esc_html($settings['not_found_text']); ?>
															</div>
														</div>
													</div>
												<?php } ?>
