...
function thegem_portfolio_item_post_type_init() {
	...
	$pf_url_slug = apply_filters('thegem_portfolio_items_rewrite_slug', 'pf');
	$cats_option = thegem_get_option('portfolio_rewrite_slug_cats');
	...
	$args = array(
		...
		'rewrite'              => array('slug' => $pf_url_slug . ($cats_option ? '/%thegem_portfolios%' : ''), 'with_front' => false),
	...
	if($cats_option) {
		add_rewrite_tag('%thegem_portfolios%', '(.+?)', 'thegem_portfolios=');
		add_rewrite_rule(
			'^' . $pf_url_slug . '/(.+?)/([^/]+)/?$',
			'index.php?thegem_pf_item=$matches[2]&thegem_portfolios=$matches[1]',
			'top'
		);
		add_rewrite_rule(
			'^' . $pf_url_slug . '/([^/]+)/?$',
			'index.php?thegem_pf_item=$matches[1]',
			'top'
		);
	}
...
function thegem_portfolio_post_type_link($permalink, $post) {
	if (!thegem_get_option('portfolio_rewrite_slug_cats') || $post->post_type !== 'thegem_pf_item') {
		return $permalink;
	}

	$terms = wp_get_object_terms($post->ID, 'thegem_portfolios');
	...
	if (!empty($terms) && !is_wp_error($terms)) {
		...
		$ancestors = get_ancestors($term->term_id, 'thegem_portfolios');
		if (!empty($ancestors)) {
			$ancestors = array_reverse($ancestors);
			foreach ($ancestors as $ancestor_id) {
				$ancestor = get_term($ancestor_id, 'thegem_portfolios');
	...
	$permalink = str_replace('%thegem_portfolios%', $slug, $permalink);
...
add_filter('post_type_link', 'thegem_portfolio_post_type_link', 10, 2);
