...
class TheGem_TemplateMenu extends Widget_Base {
	...
	public function thegem_templates_menu_search_widget($items) {
		...
		$items .= '<a href="#" role="button" aria-label="' . esc_html__('Search', 'thegem') .'"></a>';
		$items .= '<div class="minisearch">';
		$items .= '<form role="search" class="sf" action="' . esc_url(thegem_home_url('/')) . '" method="GET">';
		$items .= '<input class="sf-input" type="text" placeholder="' . esc_html__('Search...', 'thegem') . '" name="s" aria-label="' . esc_html__('Search', 'thegem') .'">';
		...
		$items .= '<input class="sf-submit" type="submit" value="" aria-label="' . esc_html__('Search', 'thegem') .'">';
		$items .= '</form>';
		$items .= '</div>';
		$items .= '</li>';
...
//					'vertical' => __('Vertical', 'thegem'),
		...
		$this->add_control(
			'hamburger_canvas_position',
			[
				'label' => __('Nav. Bar Position', 'thegem'),
				'type' => Controls_Manager::SELECT,
				'default' => 'right',
				'options' => [
					'left' => __('Left', 'thegem'),
		...
		$this->add_control(
			'menu_layout_desktop_breakpoint',
			[
				'label' => __('Desktop Breakpoint', 'thegem'),
				'type' => Controls_Manager::NUMBER,
				'min' => 0,
				'max' => 2000,
				'step' => 1,
		...
		$this->add_control(
			'menu_layout_dropdown_width',
			[
				'label' => __('Submenu Dropdown Width', 'thegem'),
				'type' => Controls_Manager::SLIDER,
				'size_units' => ['px'],
				'range' => [
					'px' => [
				...
				'selectors' => [
					'{{WRAPPER}} .thegem-te-menu__default.desktop-view ul.nav-menu > li:not(.megamenu-enable):not(.megamenu-template-enable) ul' => 'width: {{SIZE}}{{UNIT}} !important;'
				],
				'description' => __('Specify the custom width of the submenu dropdown (not applicable for mega menus)', 'thegem'),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					{{WRAPPER}} .thegem-te-menu > nav.desktop-view ul.nav-menu > li.menu-item-current > a:hover' => 'color: {{VALUE}} !important',
				],
			]
		);
		...
		$this->add_responsive_control (
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} .thegem-te-menu__hamburger.desktop-view.hamburger-position-left:not(.hamburger-active) ul.nav-menu' => 'transform: translateX(-{{SIZE}}{{UNIT}})',
					'{{WRAPPER}} .thegem-te-menu__hamburger.desktop-view.hamburger-position-left.hamburger-active .hamburger-toggle-close' => 'transform: translateX({{SIZE}}{{UNIT}})',
				],
				'condition' => [
					'menu_layout_desktop' => ['hamburger'],
				],
	...
	protected function render() {
		...
		if (!empty($settings['menu_source']) && is_nav_menu($settings['menu_source']) && !empty(wp_get_nav_menu_items($settings['menu_source']))):
			...
			$hamburger_canvas_position_class = '';
			if($menu_type === 'hamburger') {
				$hamburger_canvas_position_class = ' hamburger-position-' . ($settings['hamburger_canvas_position'] === 'left' ? 'left' : 'right');
			}
			...
			if (isset($settings['menu_layout_desktop_breakpoint']) && !empty($settings['menu_layout_desktop_breakpoint'])){
				$desktop_breakpoint = intval($settings['menu_layout_desktop_breakpoint']);
			}
			...
			<div <?php echo $this->get_render_attribute_string('thegem-te-menu-wrap'); ?>>
				<nav id="<?= esc_attr($uniqid) ?>"
					 class="desktop-view menu-class-id-<?= esc_attr($uniqid) ?> thegem-te-menu__<?php echo esc_attr($menu_type.$hamburger_canvas_position_class); ?> thegem-te-menu-mobile__<?php echo esc_attr($menu_mobile_type); ?><?php echo $menu_mobile_type == 'global-section' ? ' thegem-te-menu-mobile__slide-horizontal' : ''; ?> <?php echo esc_attr($menu_widget) ?> <?php echo esc_attr($menu_mobile_widget) ?> <?php echo esc_attr($different_source) ?>"
									...
									<div class="mobile-menu-slide-wrapper<?php echo $menu_mobile_type == 'global-section' ? ' mobile-menu-slide-global-section' : ''; ?>">
										...
										if($settings['menu_layout_desktop'] == 'hamburger' || $settings['menu_layout_desktop'] == 'overlay') {
											if($settings['hamburger_overlay_menu_source'] == 'template' && !empty($settings['hamburger_overlay_template'])) {
												$GLOBALS['thegem_offcanvas_menu_template'] = $settings['hamburger_overlay_template'];
			...
			if ($settings['menu_layout_desktop'] == 'split' && !empty($output_logo)) { ?>
				<script>
					(function ($) {
						...
						$('.elementor-element-<?php echo esc_attr($this->get_id()); ?>').closest('.elementor-widget-wrap').css('flex-wrap', 'nowrap !important');

						$('.elementor-element-<?php echo esc_attr($this->get_id()); ?> .menu-item-type-split-logo > .logo-fullwidth-block').html('<div class="site-logo" style="<?php echo esc_attr($output_logo_style); ?>"><a href="<?php echo esc_url(thegem_home_url('/'))?>" rel="home"><span class="logo"><?php echo $output_logo; ?></span></a></div>');

						$('.elementor-element-<?php echo esc_attr($this->get_id()); ?>').each(function () {
