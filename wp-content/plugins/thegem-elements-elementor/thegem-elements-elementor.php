...
function thegem_fonts_allowed_mime_types( $existing_mimes = array() ) {
	...
	$existing_mimes['ttf1'] = 'application/x-font-ttf';
	$existing_mimes['ttf2'] = 'font/sfnt';
	$existing_mimes['ttf3'] = 'application/octet-stream';
	$existing_mimes['eot'] = 'application/vnd.ms-fontobject';
	$existing_mimes['woff'] = 'application/x-font-woff';
	$existing_mimes['woff2'] = 'font/woff2';
	$existing_mimes['woff21'] = 'application/octet-stream';
...
function thegem_bypass_font_mime_check($filetype_data, $file, $filename, $mimes) {
	...
	if (isset($font_extensions[$ext])) {
		$filetype_data['ext'] = $ext;
		$filetype_data['type'] = $font_extensions[$ext];
		$filetype_data['proper_filename'] = $filename;
	}

return $filetype_data;
}
add_filter('wp_check_filetype_and_ext', 'thegem_bypass_font_mime_check', 10, 4);
				...
				foreach($elements as $widget) {
					if (isset($widget->elType) && $widget->elType == 'widget') {
						...
						if ($widget->widgetType == 'thegem-portfolio' || $widget->widgetType == 'thegem-portfolio-list') {
							...
							if(function_exists('thegem_enqueue_template_styles') && isset($widgetSettings['skin_source']) && $widgetSettings['skin_source'] === 'builder' && !empty($widgetSettings['loop_builder'])) {
...
function thegem_plugin_theme_update_notice() {
	...
	if($thegem_theme->exists() && version_compare($thegem_theme->get('Version'), '5.12.1.1', '<')) {
