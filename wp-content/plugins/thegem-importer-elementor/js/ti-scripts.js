		...
		var thegem_importer = {
			...
			init: function() {
				...
				$(document).on('click', '.thegem-importer-remover a', function(e) {
					e.preventDefault();
					that.removeDemoContentConfirm($(this).data('nonce'));
				});
				$(document).on('click', '.remove-demo-confirm', function(e) {
					e.preventDefault();
					that.removeDemoContent($(this).data('nonce'));
				});
			...
			removeDemoContentConfirm: function(nonce = '') {
				var that = this;
				$.fancybox.open(that.data.load_remove_demo_msg);
				var fbInstance = $.fancybox.getInstance();
				fbInstance.current.$smallBtn.remove();
				fbInstance.current.$smallBtn = null;
				var import_data = 'action=thegem_importer_remove_demo_confirm&nonce=' + nonce;
				$.ajax({
					url: that.data.ajax_url,
					data: import_data,
					method: 'POST',
			...
			removeDemoContent: function(nonce = '') {
				var that = this;
				$.fancybox.close();
				$.fancybox.open(that.data.load_remove_demo_msg);
				var fbInstance = $.fancybox.getInstance();
				fbInstance.current.$smallBtn.remove();
				fbInstance.current.$smallBtn = null;
				var import_data = 'action=thegem_importer_remove_demo&nonce=' + nonce;
				$.ajax({
					url: that.data.ajax_url,
					data: import_data,
					method: 'POST',
