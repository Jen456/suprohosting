...
function thegem_get_content_sliders($post_id) {
	$sliders = array();
		...
		if (is_array($data)) {
			...
			foreach($elements as $widget) {
				if (isset($widget->elType) && $widget->elType == 'widget') {
					...
					if ($widget->widgetType == 'slider_revolution' && isset($widgetSettings['shortcode'])) {
						$shortcode = $widgetSettings['shortcode'];
						preg_match("/alias=\"[a-zA-Z0-9\-\s]+\"/s", $shortcode, $matches);
						if(!empty($matches[0])) {
							$sliders[] = str_replace(array('alias="', '"'), '', $matches[0]);
						}
					}
				}
			}
